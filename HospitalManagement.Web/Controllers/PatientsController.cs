...
public class PatientsController : Controller
{
    ...
    public PatientsController(HospitalApiService api) => this.api = api;

    [HttpGet] public IActionResult Index() => View();
    [HttpGet] public async Task<IActionResult> Get(int id)
    {
        var r = await api.GetPatientAsync(id);
    ...
    {
        var r = await api.PatientsAsync(request.Search);
        return r.Ok ? Json(new { success = true, data = r.Data }) : StatusCode(502, new { message = r.Message });
    }

    [HttpPost, IgnoreAntiforgeryToken]
    public async Task<IActionResult> Save([FromBody] PatientViewModel model)
    {
        ...
        var r = model.Id == 0 ? await api.CreatePatientAsync(model) : await api.UpdatePatientAsync(model.Id, model);
        return r.Ok
            ? Json(new { success = true, message = model.Id == 0 ? "Patient added successfully." : "Patient updated successfully.", data = r.Data })
    ...
    {
        var r = await api.DeletePatientAsync(request.Id);
        return r.Ok ? Json(new { success = true, message = "Patient deleted successfully." }) : BadRequest(new { message = r.Message });
    }
}
